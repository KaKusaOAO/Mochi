...
public class RumbleEmulator
{
        ...
        float rightStrength)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            var leftVal = WaveGenerator(_leftPos) * Mth.Lerp(_leftStrO, leftStrength, i * 1f / len);
            var rightVal = WaveGenerator(_rightPos) * Mth.Lerp(_rightStrO, rightStrength, i * 1f / len);
            left[i] = leftVal;
            right[i] = rightVal;
        }
